    ...
    <div v-if="showMenu" class="w-full transition-all duration-500 lg:block">
        <div>
                        ...
                        <div class="flex gap-2">
                            <div class="flex flex-1 flex-col gap-2">
                                <div class="flex">
                                    ...
                                    <p class="flex justify-end text-xs text-gray-500">{{ item.unitOfMeasure }}</p>
                                </div>
                                <div class="flex justify-between gap-2">
                                    <div class="flex items-center gap-2">
                                        <label for="precio">Precio: </label>
                                        <input type="number" v-model.number="item.price" min="1" class="h-8 w-full rounded text-center ring-1 ring-gray-200 sm:text-sm" />
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <label for="medida">Medida: </label>
                                        <input type="text" v-model="item.unitOfMeasure" class="h-8 w-full rounded text-center ring-1 ring-gray-200 sm:text-sm" />
                                    </div>
                                </div>
                            </div>
            ...
            <div class="flex justify-center border-t p-2 sm:p-4">
                <PrimaryButton :disabled="cart.length === 0" @click="$emit('toggle-quote-modal')">Agregar Productos</PrimaryButton>
            </div>
        </div>
    </div>
</template>
