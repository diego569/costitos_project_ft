...
<template>
    <div v-for="item in displayedItems" :key="item.slug">
        <NuxtLink :to="{name: isSubcategory ? 'explorar-subcategoria-slug' : 'explorar-categoria-slug', params: {slug: item.slug}}" class="flex h-16 cursor-pointer items-center rounded-xl border border-gray-200 bg-white bg-clip-border text-gray-700 transition-all duration-300 hover:shadow-md">
